...
plt.figure()
cp = plt.contourf(Z,X, Y )
plt.colorbar(cp)

plt.title('Contour Plot Distributed Load on Airleron [kN/m^2]')
plt.xlabel('Z along chord [m]')
plt.ylabel('X along wingspan [m]')
plt.show()
...
ax.set_xlabel('Z along chord [m]')
ax.set_ylabel('X along wingspan [m]')
ax.set_zlabel('Distributed Load [kN/m^2]')
ax.view_init(azim=180+190+12)
plt.show()